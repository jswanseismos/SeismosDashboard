...
-- --------------------------------------------------
-- Date Created: 07/17/2018 15:28:18
...
GO
IF OBJECT_ID(N'[dbo].[FK_CylinderMaterial]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[Cylinders] DROP CONSTRAINT [FK_CylinderMaterial];
GO
...
GO
IF OBJECT_ID(N'[dbo].[CasingChartLookup]', 'U') IS NOT NULL
    DROP TABLE [dbo].[CasingChartLookup];
GO
...
CREATE TABLE [dbo].[Wells] (
    ...
    [DeviationSurveys_Id] uniqueidentifier  NOT NULL,
    [WellBore_Id] uniqueidentifier  NOT NULL,
    [Wellhead_Id] uniqueidentifier  NOT NULL
);
GO
...
-- Creating foreign key on [DeviationSurveys_Id] in table 'Wells'
ALTER TABLE [dbo].[Wells]
ADD CONSTRAINT [FK_WellsDeviationSurveys]
    FOREIGN KEY ([DeviationSurveys_Id])
...
-- Creating foreign key on [WellBore_Id] in table 'Wells'
ALTER TABLE [dbo].[Wells]
...
-- Creating foreign key on [Wellhead_Id] in table 'Wells'
ALTER TABLE [dbo].[Wells]
ADD CONSTRAINT [FK_WellWellhead]
    FOREIGN KEY ([Wellhead_Id])
    REFERENCES [dbo].[Wellheads]
        ([Id])
    ON DELETE NO ACTION ON UPDATE NO ACTION;
GO
